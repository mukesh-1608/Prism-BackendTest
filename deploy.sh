...
server {
    server_name $REPO_NAME-app.$DOMAIN;
...
server {
    server_name $REPO_NAME-backend.$DOMAIN;
...
setup_nginx_config() {
    ...
    sudo cp certs/frontend.conf /etc/nginx/sites-available/$REPO_NAME-app.$DOMAIN
    
    # Copy backend configuration to sites-available
    print_status "Copying backend nginx configuration..."
    sudo cp certs/backend.conf /etc/nginx/sites-available/$REPO_NAME-backend.$DOMAIN
    
    # Create symbolic links to sites-enabled
    print_status "Creating symbolic links..."
    sudo ln -sf /etc/nginx/sites-available/$REPO_NAME-app.$DOMAIN /etc/nginx/sites-enabled/
    sudo ln -sf /etc/nginx/sites-available/$REPO_NAME-backend.$DOMAIN /etc/nginx/sites-enabled/
...
generate_ssl_certificates() {
    ...
    print_status "Generating certificate for $REPO_NAME-app.$DOMAIN..."
    certbot certonly --standalone -d "$REPO_NAME-app.$DOMAIN" --non-interactive --agree-tos --email admin@$DOMAIN
    ...
    print_status "Generating certificate for $REPO_NAME-backend.$DOMAIN..."
    certbot certonly --standalone -d "$REPO_NAME-backend.$DOMAIN" --non-interactive --agree-tos --email admin@$DOMAIN
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/$REPO_NAME-app.$DOMAIN/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/$REPO_NAME-app.$DOMAIN/privkey.pem; # managed by Certbot
}
server {
    if (\$host = $REPO_NAME-app.$DOMAIN) {
...
server {
    ...
    listen 443 ssl; # managed by Certbot
    ssl_certificate /etc/letsencrypt/live/$REPO_NAME-backend.$DOMAIN/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/$REPO_NAME-backend.$DOMAIN/privkey.pem; # managed by Certbot
}
server {
    if (\$host = $REPO_NAME-backend.$DOMAIN) {
        return 301 https://\$host\$request_uri;
    } # managed by Certbot
...
EOF
    ...
    sudo cp /tmp/frontend_ssl.conf /etc/nginx/sites-available/$REPO_NAME-app.$DOMAIN
    sudo cp /tmp/backend_ssl.conf /etc/nginx/sites-available/$REPO_NAME-backend.$DOMAIN
    ...
    cp /etc/nginx/sites-available/$REPO_NAME-app.$DOMAIN certs/frontend.conf
    cp /etc/nginx/sites-available/$REPO_NAME-backend.$DOMAIN certs/backend.conf
...
copy_certificates_to_s3() {
    ...
    aws s3api put-object --bucket "$S3_BUCKET_NAME" --key "certificates/$REPO_NAME/$REPO_NAME-app.$DOMAIN/" --region "$AWS_REGION" 2>/dev/null || true
    aws s3api put-object --bucket "$S3_BUCKET_NAME" --key "certificates/$REPO_NAME/$REPO_NAME-backend.$DOMAIN/" --region "$AWS_REGION" 2>/dev/null || true
    
    # Copy frontend certificates
    print_status "Copying frontend certificates..."
    # Get the latest certificate version from archive
    local frontend_fullchain=$(ls -t /etc/letsencrypt/archive/$REPO_NAME-app.$DOMAIN/fullchain*.pem | head -1)
    local frontend_privkey=$(ls -t /etc/letsencrypt/archive/$REPO_NAME-app.$DOMAIN/privkey*.pem | head -1)
    ...
    aws s3 cp "$frontend_fullchain" "s3://$S3_BUCKET_NAME/certificates/$REPO_NAME/$REPO_NAME-app.$DOMAIN/fullchain.pem" --region "$AWS_REGION"
    aws s3 cp "$frontend_privkey" "s3://$S3_BUCKET_NAME/certificates/$REPO_NAME/$REPO_NAME-app.$DOMAIN/privkey.pem" --region "$AWS_REGION"
    
    # Copy backend certificates
    print_status "Copying backend certificates..."
    # Get the latest certificate version from archive
    local backend_fullchain=$(ls -t /etc/letsencrypt/archive/$REPO_NAME-backend.$DOMAIN/fullchain*.pem | head -1)
    local backend_privkey=$(ls -t /etc/letsencrypt/archive/$REPO_NAME-backend.$DOMAIN/privkey*.pem | head -1)
    ...
    aws s3 cp "$backend_fullchain" "s3://$S3_BUCKET_NAME/certificates/$REPO_NAME/$REPO_NAME-backend.$DOMAIN/fullchain.pem" --region "$AWS_REGION"
    aws s3 cp "$backend_privkey" "s3://$S3_BUCKET_NAME/certificates/$REPO_NAME/$REPO_NAME-backend.$DOMAIN/privkey.pem" --region "$AWS_REGION"
...
commit_changes() {
    ...
    - Generated SSL certificates for $REPO_NAME-app.$DOMAIN and $REPO_NAME-backend.$DOMAIN
...
update_terraform_folder() {
    ...
    #mkdir $REPO_NAME/Terraform
    cp -r /tmp/Terraform "$REPO_NAME/"
...
create_iac_branch() {
    print_status "Creating and publishing iac branch..."
    ...
    print_status "Detected default branch: $default_branch"
    
    # Ensure we're on the default branch and it's up to date
    print_status "Switching to $default_branch branch and pulling latest changes..."
    git checkout "$default_branch"
    git pull origin "$default_branch"
    
    # Check if iac branch already exists
    if git show-ref --verify --quiet refs/heads/iac; then
        print_warning "iac branch already exists. Deleting it to recreate from $default_branch..."
        git branch -D iac
        git push origin --delete iac 2>/dev/null || true
    fi
    
    # Create new iac branch from default branch
    print_status "Creating new iac branch from $default_branch..."
    git checkout -b iac
